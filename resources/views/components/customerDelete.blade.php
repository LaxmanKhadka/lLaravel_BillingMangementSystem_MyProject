    ...
    <div class="row">
        <div class="col-md-10">
            <div class="card">
                ...
                <form class="form-horizontal" method="post" action="#" id="customerDelete">

                    @csrf
                    <input type="hidden" value="{{ $v->id }}" name="id" id="id">
                    <div class="card-body">
                        <div class="deleteCustomer" style="display: block">
                            ...
                            <div class="form-group row">
                                <div class="col-sm-12">
                                    <label for="address" class="text-end control-label col-form-label">City
                                    </label>
                                    <span class="req" style="color:red"></span>
                            ...
                            <div class="form-group row">
                                <div class="col-sm-12">
                                    ...
                                    <select name="Country" id="Country" class="require">
                                        @foreach ($dat as $i => $va)
                                            <option value="{{ $va }}"
                                                @if ($va == $v->Country) selected @endif>
                                                {{ $va }}</option>
                                        @endforeach
                            ...
                            <div class="border-top">
                                <div class="card-body" style="float:right">
                                    <button type="button" id="Delete" name="button"
                                        class="btn btn-danger btn-lg btn-outline-danger">
                                        Delete
                                    </button>
                                </div>
                            </div>
...
<script>
    ...
    $("#Delete").on("click", function() {
        ...
        if (check == 0) {
            var data = $("#customerDelete").serialize();

            console.log(data);

            $("#productModelClose").click();
            alert("New Customer Deleted Sucessfully !");

            ajax("/customerDeleted", data, "result", "post", productrefresh);
        }
    });
</script>
